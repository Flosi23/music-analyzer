...
export default function SelectPage({setSelection}: Props) {
    const {
        data: playlists,
        loading,
        error,
    ...
    const select = () => {
        ...
        setSelection({
            includeLikedSongs,
            playlistIds: playlists.map((p) => p.id).filter((p): p is string => p !== undefined)
        })
    }
        ...
        <>
            ...
            <div className="flex flex-row gap-10 items-start">
                <div className="flex flex-col w-1/3">
                    ...
                    />
                    <Button onClick={select} className="mt-8 w-full">
